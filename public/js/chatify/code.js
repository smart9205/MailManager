...
function updateSelectedContact1(user_id) {
  $(document).find(".messenger-list_item").removeClass("m-list-active");
  $(document)
    .find(
      ".messenger-list_item[data-contact=" + (user_id || getMessengerId()) + "]"
    )
    .addClass("m-list-active");
}
...
function IDinfo1(id, domain_id, type) {
  // clear temporary message id
  temporaryMsgId = 0;
  // clear typing now
  typingNow = 0;
  ...
  if (messenger != 0) {
    ...
    $.ajax({
      url: url + "/idInfo",
      method: "POST",
      data: { _token: access_token, id, type,domain_id },
      dataType: "JSON",
      success: (data) => {
        // avatar photo
        ...
        $(".messenger-infoView-shared").show();
        // fetch messages
        fetchMessages1(id, type,domain_id, true);
        // focus on messaging input
        messageInput.focus();
        // update info in view
        $(".messenger-infoView .info-name").html(data.fetch.name);
...
function fetchMessages1(id, type,domain_id, newFetch = false) {
  if (newFetch) {
    messagesPage = 1;
    noMoreMessages = false;
  }
...
$(document).ready(function () {
  ...
  $("body").on("click", ".messenger-list_item", function () {
    const dataView = $(".messenger-list_item")
      .find("p[data-type]")
      .attr("data-type");
    $(".messenger-tab").hide();
    $(".messenger-tab[data-view=" + dataView + "s]").show();

    $(".messenger-list_item").removeClass("m-list-active");
    $(this).addClass("m-list-active");
    const userID = $(this).attr("data-contact");
    updateSelectedContact1(userID);
  });
  ...
  $("body").on("click", ".messenger-list_item", function () {
    ...
    var domain_id = pathArray[4];
    
    setMessengerId(dataId);
    setMessengerType(dataType);
    IDinfo1(dataId, domain_id, dataType);
  });
