...
@section('content')
    <a href="{{url("customer/user/add/". $domain_id)}}" class="bg-indigo-500 hover:bg-indigo-600 relative shadow-xl rounded-md p-2 mb-12 lg:mb-3 text-gray-100">
        Add User
    </a>
...
@section('scripts')
    ...
    $(function() {
        $('#users-tbl').DataTable({
            processing: true,
            serverSide: true,
            ajax      : '{!! url('/customer/fetch/users/' . $domain_id) !!}', 
