...
<section class="">
    ...
    <form method="POST" action="{{ url('/admin/counsellor/update') }}" class="mx-auto container max-w-2xl md:w-3/4 shadow-xl">
      ...
      <div class="bg-gray-100 p-4 border-t-2 bg-opacity-5 border-indigo-400 rounded-t">
        <h2 class="font-bold text-2xl my-3">Edit Customers</h2>
        <hr>
        <div class="max-w-sm mx-auto md:w-full md:mx-0">
          <div class="inline-flex items-center">
            {{-- <h1 class="text-gray-600 font-semibold mt-2">{{($customer->last_name)}} {{($customer->first_name)}}</h1> --}}
          </div>
        </div>
      </div>
      <div class="bg-white space-y-6">
        ...
        <div class="md:inline-flex  space-y-4 md:space-y-0  w-full p-4 text-gray-500 items-center">
          ...
          <div class="md:w-2/3 mx-auto max-w-sm space-y-5">
            ...
            <div>
              <label class="text-sm text-gray-400">Email Limit</label>
              <div class="w-full inline-flex border">
                <div class="pt-2 w-1/12 bg-gray-100 bg-opacity-50">
                  <svg
                    fill="none"
        ...
        <hr />
        <div class="md:inline-flex  space-y-4 md:space-y-0  w-full p-4 text-gray-500 items-center">
          <h2 class="md:w-1/3 mx-auto max-w-sm">Customer info</h2>
          <div class="md:w-2/3 mx-auto max-w-sm space-y-5">
            <div>
              ...
              <div class="w-full inline-flex border">
                ...
                <select class="w-11/12 focus:outline-none focus:text-gray-600 p-2" name="is_active" required>
                  @foreach (ActiveStatus::getAll() as $key => $value)
                    <option value="{{$key}}" {{ ( $key == $customer->is_active) ? 'selected' : '' }}>{{$value}}</option>
                  @endforeach
                </select>
              </div>
              @error('is_active')
